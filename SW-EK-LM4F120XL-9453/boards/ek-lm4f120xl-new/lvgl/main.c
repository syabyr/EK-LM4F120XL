...
static void btn_color_event_cb(lv_event_t * e)
{
    ...
    lv_label_set_text_fmt(status_label, "R%02X G%02X B%02X", c32.red, c32.green, c32.blue);
...
static void create_demo_ui(void)
{
    ...
    lv_obj_t * lbl_r = lv_label_create(btn_red);
    lv_label_set_text(lbl_r, "Red"); lv_obj_center(lbl_r);
    ...
    lv_obj_t * lbl_g = lv_label_create(btn_green);
    lv_label_set_text(lbl_g, "Green"); lv_obj_center(lbl_g);
    ...
    lv_obj_t * lbl_b = lv_label_create(btn_blue);
    lv_label_set_text(lbl_b, "Blue"); lv_obj_center(lbl_b);
...
int main(void)
{
    ...
    /* Force first render — lv_timer_handler hangs if the first
     * refresh is triggered before any tick is available. */
    lv_refr_now(NULL);
    printf("LVGL running\r\n");
